...
async def stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total_users, total_points = db.get_stats()
    # CRIT-02 FIX: STATS_MSG now requires {task_count} — must be fetched and passed
    task_count = db.get_active_task_count()
    keyboard = [[InlineKeyboardButton("🔙 العودة", callback_data="admin_main")]]
    await query.edit_message_text(
        get_str(c.ADMIN_ID, 'STATS_MSG').format(total_users=total_users, total_points=total_points, task_count=task_count),
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown"
    )
